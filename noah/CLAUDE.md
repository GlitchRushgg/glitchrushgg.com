...
- **Boot** ([src/scenes/BootScene.js](src/scenes/BootScene.js)) — loads PNG assets from `assets/` and **procedurally generates** the `star` and `heart` textures via `Graphics.js` + `generateTexture`. All 8 animals ship as hand-drawn pixel-art PNGs in `assets/images/` and are loaded in `preload()` (see the `ANIMALS` list). Animal sprite keys: `elephant, giraffe, lion, zebra, monkey, rabbit, penguin, bear`. (`drawAnimal` in `Graphics.js` is now unused legacy.)
