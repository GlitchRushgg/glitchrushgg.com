...
// gavetas ABIERTAS: misma identidad que las cerradas (referencia = la imagen ya generada)
const refB64 = "data:image/png;base64," + readFileSync(join(srcDir, "drawers2.png")).toString("base64");
const prompt = "Same kitchen cabinet as the reference image, same colours, same size, same camera angle and same position in frame — but now the THREE DRAWERS ARE PULLED OPEN toward the viewer, each one a little further out than the one above, so you can see kitchen utensils inside them (a wooden spoon, a whisk, a small pot). Dark shadow inside each open drawer. Centered with generous margin on a SOLID FLAT PURE BRIGHT GREEN chroma-key background (#00ff00). No floor, no shadow on the ground, no text. STYLE: cute stylized 3D video game prop render, Fortnite/Pixar toy-like look, matte vibrant colors, kid-friendly. AVOID: photorealism, halloween, dark, text, watermark.";
if (!existsSync(join(srcDir, "drawers2-open-cut.png"))) {
  const res = await fetch("https://api.replicate.com/v1/models/google/nano-banana/predictions", {
    ...
    body: JSON.stringify({ input: { prompt, image_input: [refB64], aspect_ratio: "1:1", output_format: "png" } }),
  });
  const body = await res.json();
  const url = Array.isArray(body.output) ? body.output[0] : body.output;
  if (!url) { console.error("FALLO " + JSON.stringify(body).slice(0, 300)); process.exit(1); }
  const img = await fetch(url);
  writeFileSync(join(srcDir, "drawers2-open.png"), Buffer.from(await img.arrayBuffer()));
  console.log("GEN drawers2-open");
  cut("drawers2-open");
}
